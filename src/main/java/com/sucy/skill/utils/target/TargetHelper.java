...
public abstract class TargetHelper {
    ...
    public static List<LivingEntity> getLivingTargets(LivingEntity source, double range, double tolerance) {
        ...
        NRay3D ray = new NRay3D(location);
        ...
        for (Entity entity : list) {
            ...
            NVec3D collision = aabb.intersectsRay(ray, 0, range);
            if (collision != null) {
    ...
    public static AABB getAABB(Entity entity) {
        NVec3D origin = NVec3D.fromLocation(entity.getLocation());
        try {
            if (PluginChecker.isDisguiseActive() && DisguiseAPI.isDisguised(entity) && DisguiseAPI.getDisguise(entity).isModifyBoundingBox()) {
                ...
                return new AABB(origin.add(new NVec3D(-boundingBox.getX(), 0, -boundingBox.getZ())), origin.add(new NVec3D(boundingBox.getX(), boundingBox.getY(), boundingBox.getZ())));
            }
        } catch (NullPointerException ignored) {
        }
        double halfWidth = entity.getWidth() / 2;
        return new AABB(origin.add(new NVec3D(-halfWidth, 0, -halfWidth)), origin.add(new NVec3D(halfWidth, entity.getHeight(), halfWidth)));
    }
