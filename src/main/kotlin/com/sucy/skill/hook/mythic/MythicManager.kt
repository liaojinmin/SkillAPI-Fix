...
object MythicManager: Listener {
    ...
    fun onStart() {
        ...
        Bukkit.getScheduler().runTaskTimer(SkillAPI.singleton(), MythicManager::tick, 20, 20)
    }
    ...
    private fun tick() {
        summonMap.forEach { (key, value) ->
            try {
                // 先删除过期
                value.summon.values.removeIf { it.checkTimerOut() || value.owner.world.name != it.activeMob.entity.world.name }
